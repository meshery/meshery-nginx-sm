	...
	for {
		...
		case data := <-s.Channel:
			event := &meshes.EventsResponse{
				OperationId: data.(*nginx.Event).Operationid,
				EventType:   meshes.EventType(data.(*nginx.Event).EType),
				Summary:     data.(*nginx.Event).Summary,
				Details:     data.(*nginx.Event).Details,
			}
